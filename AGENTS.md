...
- Diagram exports must pass `archify validate --quality standard` unchanged; `showcase` also forbids crossings, which a dependency graph with hubs cannot avoid, so it is for hand-tuned diagrams only. Curate before drawing: top-N nodes by the area's metric with the project's own nodes first, the rest collapsed, at most five views, one node per row; the full graph goes to DOT and Mermaid. The committed fixtures under `Tests/Fixtures/archify` are what CI validates with the real archify at the pinned commit in `archify-version.json`; regenerate them with `DiagramFixtures.Update` after an intentional exporter change and bump the pin and the mirrored schemas together.
