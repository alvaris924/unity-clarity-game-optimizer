...
- Diagrams: the Architecture scan now also writes the assembly map as archify JSON (`assemblies.architecture.json`, an `architecture` diagram at the `standard` quality profile: the project's own assemblies first, a quarter of the twelve slots for the packages and plugins they use most, one component per row so nothing routes through a node, group as a tag, line weight for folded references, a card with the findings above Info), as Graphviz DOT and as Mermaid (the whole graph, grouped), and renders the HTML with archify when it is found through `Tools > Clarity Game Optimizer > Locate archify`, `ARCHIFY_HOME` or an agent skill folder and Node.js is on the path. A `Diagrams` workflow validates the committed fixtures with the pinned archify release on every pull request.
